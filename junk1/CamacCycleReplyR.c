/*
 * Generated by asn1c-0.9.28 (http://lionet.info/asn1c)
 * From ASN.1 module "EPICS2CAMAC"
 * 	found in "../epics2camac.asn1"
 * 	`asn1c -pdu=all -fincludes-quoted -fline-refs -fall-defs-global -findirect-choice -gen-PER -R`
 */

#include "CamacCycleReplyR.h"

static asn_TYPE_member_t asn_MBR_CamacCycleReplyR_1[] = {
	{ ATF_NOFLAGS, 0, offsetof(struct CamacCycleReplyR, requestR),
		(ASN_TAG_CLASS_CONTEXT | (0 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_BasicCycleRequest,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"requestR"
		},
	{ ATF_NOFLAGS, 0, offsetof(struct CamacCycleReplyR, status),
		(ASN_TAG_CLASS_CONTEXT | (1 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_NativeInteger,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"status"
		},
	{ ATF_NOFLAGS, 0, offsetof(struct CamacCycleReplyR, rdata),
		(ASN_TAG_CLASS_CONTEXT | (2 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_NativeInteger,
		0,	/* Defer constraints checking to the member type */
		0,	/* No PER visible constraints */
		0,
		"rdata"
		},
};
static const ber_tlv_tag_t asn_DEF_CamacCycleReplyR_tags_1[] = {
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static const asn_TYPE_tag2member_t asn_MAP_CamacCycleReplyR_tag2el_1[] = {
    { (ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0 }, /* requestRat 51 */
    { (ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0 }, /* statusat 52 */
    { (ASN_TAG_CLASS_CONTEXT | (2 << 2)), 2, 0, 0 } /* rdataat 54 */
};
static asn_SEQUENCE_specifics_t asn_SPC_CamacCycleReplyR_specs_1 = {
	sizeof(struct CamacCycleReplyR),
	offsetof(struct CamacCycleReplyR, _asn_ctx),
	asn_MAP_CamacCycleReplyR_tag2el_1,
	3,	/* Count of tags in the map */
	0, 0, 0,	/* Optional elements (not needed) */
	-1,	/* Start extensions */
	-1	/* Stop extensions */
};
asn_TYPE_descriptor_t asn_DEF_CamacCycleReplyR = {
	"CamacCycleReplyR",
	"CamacCycleReplyR",
	SEQUENCE_free,
	SEQUENCE_print,
	SEQUENCE_constraint,
	SEQUENCE_decode_ber,
	SEQUENCE_encode_der,
	SEQUENCE_decode_xer,
	SEQUENCE_encode_xer,
	SEQUENCE_decode_uper,
	SEQUENCE_encode_uper,
	0,	/* Use generic outmost tag fetcher */
	asn_DEF_CamacCycleReplyR_tags_1,
	sizeof(asn_DEF_CamacCycleReplyR_tags_1)
		/sizeof(asn_DEF_CamacCycleReplyR_tags_1[0]), /* 1 */
	asn_DEF_CamacCycleReplyR_tags_1,	/* Same as above */
	sizeof(asn_DEF_CamacCycleReplyR_tags_1)
		/sizeof(asn_DEF_CamacCycleReplyR_tags_1[0]), /* 1 */
	0,	/* No PER visible constraints */
	asn_MBR_CamacCycleReplyR_1,
	3,	/* Elements count */
	&asn_SPC_CamacCycleReplyR_specs_1	/* Additional specs */
};

